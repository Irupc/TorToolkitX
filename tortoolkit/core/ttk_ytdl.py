...
async def handle_ytdl_command(e: MessageLike):
    if not e.is_reply:
        await e.reply(
            "ඔය Command එක ඔයාට Upload කරන්න ඕනි YouTube Video එකේ Link එකට Reply එකක් විදිහට දාන්න"
        )
        return
...
async def handle_ytdl_playlist(e: MessageLike) -> None:
    if not e.is_reply:
        await e.reply(
            "ඔය Command එක ඔයාට Upload කරන්න ඕනි YouTube Playlist එකේ Link එකට Reply එකක් විදිහට දාන්න"
        )
        return
